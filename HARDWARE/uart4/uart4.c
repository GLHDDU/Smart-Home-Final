...
void UART4_IRQHandler(void)
{
	if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)  //接收到数据
	{
		...
		if(UART4_RX_BUF[UART4_RX_COUNT-1]=='*' || UART4_RX_COUNT>UART4_REC_LEN)//缓存区溢出
		F_UART4_RX_FINISH=1;//UART4接收完成
		//end 接收未完成   		 		 
	}	//end 接收到数据
}
...
void parse_cmd(void)
{
	if(F_UART4_RX_FINISH) //数据接收完成才处理
	{		
			if( strstr( (char*)UART4_RX_BUF,"OPEN") )
			{
			...
			//识别失败
			if( strstr( (char*)UART4_RX_BUF,"fail") )
			{

				printf("face recognition fail\r\n");

			}
				
		F_UART4_RX_FINISH=0;   //处理完成后清除标志位
		//清空缓冲区
